...
import android.text.Spannable;                      // 안전지도 색칠
import android.text.SpannableString;                // 안전지도 색칠
import android.text.style.ForegroundColorSpan;      // 안전지도 색칠
...
import com.example.eyekeep.request.RequestRoute;
import com.example.eyekeep.request.RequestSignout;
import com.example.eyekeep.retrofit.RetrofitClient;
import com.example.eyekeep.service.LocationUpdateService;
...
import com.example.eyekeep.bookmark.RequestChildBookMark;

import com.example.eyekeep.service.AcceptEyeKeep;
import com.example.eyekeep.bluetooth.BluetoothParingActivity;
...
public class MainChildActivity extends AppCompatActivity implements OnMapReadyCallback {

    private FragmentManager fragmentManager;
    private MapFragment mapFragment;
    private AppCompatImageButton mbtn_map_my_location, btneyekeeplocation;
    ...
    private AppCompatButton safeMapButton, btnbluetooth, bookmarkMenuButton, myInformationButton, startDirectionButton;
    ...
    private final RequestRoute requestRoute = new RequestRoute(MainChildActivity.this);

    private AcceptEyeKeep acceptEyeKeep;
    private BookmarkViewModel bookmarkViewModel;
    private RequestChildBookMark requestBookMark;  // 북마크 리스트 관리 객체
    private BookmarkChildAdapter bookmarkAdapter;
    ...
    private Drawable safeMapButtonDrawable; // drawableTop이 [1]에 위치
    private Drawable bookmarkButtonDrawable;
    private Drawable myInformationButtonDrawable;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        requestBookMark = new RequestChildBookMark(this, bookmarkViewModel);
        ...
        myInformationButton = findViewById(R.id.btn_my_info); //내정보 버튼

        safeMapButtonDrawable = safeMapButton.getCompoundDrawables()[1]; // drawableTop이 [1]에 위치
        bookmarkButtonDrawable = bookmarkMenuButton.getCompoundDrawables()[1];
        myInformationButtonDrawable = myInformationButton.getCompoundDrawables()[1];
        defaultColor = ContextCompat.getColor(MainChildActivity.this, R.color.black);
        ...
        menuViewModel.getCurrentMenu().observe(this, menuType -> {
            ...
            switch (menuType) {
                case SAFETY_MAP:
                    ...
                    safeMapButtonDrawable.setTint(ContextCompat.getColor(MainChildActivity.this, R.color.blue)); // 원하는 색상으로 변경
                    safeMapButton.setTextColor(ContextCompat.getColor(this, R.color.blue));
                    break;
                case MY_INFORMATION:
                    overlayContainer.addView(myInformationMenu);
                    myInformationButtonDrawable.setTint(ContextCompat.getColor(MainChildActivity.this, R.color.blue)); // 원하는 색상으로 변경
                    myInformationButton.setTextColor(ContextCompat.getColor(this, R.color.blue));
                    break;
                case BOOKMARK:
                    overlayContainer.addView(bookmarkMenu);
                    bookmarkButtonDrawable.setTint(ContextCompat.getColor(MainChildActivity.this, R.color.blue)); // 원하는 색상으로 변경
        ...
        RequestSignout requestSignout = new RequestSignout(MainChildActivity.this);
        ...
        btnbluetooth.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(MainChildActivity.this, BluetoothParingActivity.class);
                startActivity(intent);
            }
        });

        acceptEyeKeep = new AcceptEyeKeep(MainChildActivity.this);
    ...
    public void setBookmarkAdapter() {
        ...
        bookmarkAdapter = new BookmarkChildAdapter(bookmarkViewModel.getBookmarkList().getValue(), requestBookMark, MainChildActivity.this);
    ...
    private void showRequestDialog(String nickname, String email) {
        ...
        // 커스텀 다이얼로그 레이아웃을 불러옵니다.
        LayoutInflater inflater = LayoutInflater.from(this);
        View dialogView = inflater.inflate(R.layout.diolog_eyekeep_accept, null);
        ...
        // 다이얼로그 내부의 뷰 설정
        TextView requestMessage = dialogView.findViewById(R.id.requestMessage);
        AppCompatButton yesButton = dialogView.findViewById(R.id.btn_eyekeep_yes);
        AppCompatButton noButton = dialogView.findViewById(R.id.btn_eyekeep_no);

        // FCM에서 받은 닉네임과 이메일을 다이얼로그 메시지에 설정
        requestMessage.setText(nickname + " (" + email + ")님으로부터 아이킵 요청이 왔습니다.\n수락하시겠습니까?");

        // "네" 버튼 클릭 시 처리
        yesButton.setOnClickListener(v -> {
            acceptEyeKeep.acceptEyeKeep(email);
            Toast.makeText(this, nickname + "님의 요청을 수락했습니다.", Toast.LENGTH_SHORT).show();
            dialog.dismiss();
        ...
        noButton.setOnClickListener(view -> {
            Toast.makeText(this, "요청을 거부했습니다", Toast.LENGTH_SHORT).show();
            dialog.dismiss();
        });
    }
        ...
        call.enqueue(new Callback<SearchNaverDTO>() {
            ...
            public void onResponse(@NonNull Call<SearchNaverDTO> call, @NonNull Response<SearchNaverDTO> response) {
                if (!response.isSuccessful()) {
                    ...
                    Toast.makeText(MainChildActivity.this, "Response Error: Code " + statusCode + " - " + errorMessage, Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                for (SearchNaverDTO.Item item : items) {
                    try {
                        ...
                        Toast.makeText(MainChildActivity.this, "좌표 변환 오류", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
            ...
            public void onFailure(@NonNull Call<SearchNaverDTO> call, @NonNull Throwable t) {
                ...
                Toast.makeText(MainChildActivity.this, "네트워크 에러", Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        call.enqueue(new Callback<SearchGeocodingDTO>() {
            ...
            public void onResponse(@NonNull Call<SearchGeocodingDTO> call, @NonNull Response<SearchGeocodingDTO> response) {
                ...
                if (items == null || items.isEmpty()) {
                    ...
                    Toast.makeText(MainChildActivity.this, "요청 검색 결과가 없습니다.", Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                Toast.makeText(MainChildActivity.this, displayAddress + " 검색 결과", Toast.LENGTH_LONG).show();
            }
    ...
    // 새로운 메서드 오버로드
    public void fetchSearchSuggestions(String query, OnSuggestionsFetchedListener listener) {
        SearchService service = RetrofitClient.getRetrofitInstanceSearchNaver().create(SearchService.class);
    ...
    // 메서드 오버로드
    public void searchAddress(String query, OnSuggestionsFetchedListener listener) {
        SearchService service = RetrofitClient.getRetrofitInstanceSearchGeocoding().create(SearchService.class);
        Call<SearchGeocodingDTO> call = service.searchAddress(BuildConfig.NAVER_MAP_CLIENT_ID, BuildConfig.NAVER_MAP_API_KEY, query, 1);
        call.enqueue(new Callback<SearchGeocodingDTO>() {
            @Override
    ...
    private void fetchLocationAndMoveCamera(String query) {
        ...
        call.enqueue(new Callback<SearchNaverDTO>() {
            ...
            @Override
            public void onFailure(@NonNull Call<SearchNaverDTO> call, @NonNull Throwable t) {
                Toast.makeText(MainChildActivity.this, "위치 정보 가져오기 실패: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void requestDirection(FindPathDTO departureDTO, FindPathDTO arrivalDTO) {
        ...
        requestRoute.getRouteFromServer(startToEnd);
    ...
    private void exitDirectionMode() {
        // 길찾기 모드 종료 로직
        // 경로와 화살표 제거, UI 업데이트 등을 수행합니다.
        if (polyline != null) {
            polyline.setMap(null);        // 경로 폴리라인 제거
        }
        if (arrowheadPathOverlay != null) {
            arrowheadPathOverlay.setMap(null); // 화살표 경로 제거
        }
        // 다른 종료 관련 작업 수행...
    }
...
//    }
    ...
    private void startLocationService() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            startForegroundService(serviceIntent);  // 안드로이드 O 이상에서는 Foreground 서비스로 시작
        } else {
    ...
    public void onMapReady(@NonNull NaverMap naverMap) {
        ...
        fetchPoliceOffice = new FetchPoliceOffice(MainChildActivity.this, naverMap);
        fetchCCTV = new FetchCCTV(MainChildActivity.this, naverMap);
        fetchChildrenGuardHouse = new FetchChildrenGuardHouse(MainChildActivity.this, naverMap);
        fetchSecurityLamp = new FetchSecurityLamp(MainChildActivity.this, naverMap);
        fetchAccidentBlackSpot = new FetchAccidentBlackSpot(MainChildActivity.this, naverMap);
        fetchchildrenProtectionZone = new FetchChildrenProtectionZone(MainChildActivity.this, naverMap);
        fetchSafetyEmergencyBell = new FetchSafetyEmergencyBell(MainChildActivity.this, naverMap);
